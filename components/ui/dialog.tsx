...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-50 bg-charcoal-900/80 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    {...props}
  />
...
// Enhanced Modal Component with Framer Motion
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div
            className={cn(
              "fixed left-[50%] top-[50%] z-50 grid w-full translate-x-[-50%] translate-y-[-50%] gap-4 border-2 border-charcoal-200 dark:border-charcoal-700 bg-card p-6 shadow-2xl rounded-2xl",
              sizeClasses[size],
              className
            )}
            initial={{ 
          ...
          >
            ...
            {showClose && (
              <button
                onClick={onClose}
                className="absolute right-4 top-4 rounded-xl opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 p-1"
              >
                <X className="h-4 w-4" />
                <span className="sr-only">Close</span>
              </button>
